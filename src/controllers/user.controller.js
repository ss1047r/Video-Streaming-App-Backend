...
const updateAvatar = asyncHandler(async (req, res) => {
  ...
  if (!avatar.url) {
    throw new ApiError(400, "Error while uploaading avatar");
  }
  ...
  const deletePreviousAvatar = await user
    .findByIdAndUpdate(
      req.user?._id,
      {
        $set: {
  ...
  if (!deletePreviousAvatar) {
    throw new ApiError(400, "Avatar was not changed or could not be deleted");
  }
...
const getUserChannelProfile = asyncHandler(async (req, res) => {
  ...
  if (!username?.trim()) {
    throw new ApiErrorError(400, "Username is missing");
  }
